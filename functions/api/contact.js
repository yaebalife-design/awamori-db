/**
 * お問い合わせの受け口（Cloudflare Pages Functions）— 泡盛DB
 *
 * Gin-DB / 南部鉄瓶-DB と同じ方式（2026/09/05 社長「GIN-DBと同じ方式にしてほしい」）。
 *
 * ■ 設計の要点：メールアドレスがシステムのどこにも存在しない
 *   ブラウザが話す相手は awamori-db.com/api/contact だけ。
 *   受け取った内容は Google スプレッドシート「問い合わせ」の「泡盛DB」タブに1行追記し、
 *   同時に Cloudflare D1（同じアカウント内のデータベース）に控えを残す。
 *   メール送信も外部サービスへの転送もしないので、**宛先アドレスというものが存在しない**。
 *
 * ■ Cloudflare 側の設定（Pages プロジェクト awamori-db に設定済み）
 *   D1 バインディング  DB               … 控え（awamori-db-contact）
 *   環境変数           SHEETS_ID / SHEETS_TAB / GOOGLE_SA_EMAIL / GOOGLE_SA_KEY（暗号化）
 *   環境変数（任意）   CONTACT_WEBHOOK  … 外部へも飛ばしたい場合だけ。既定では使わない
 *   環境変数（任意）   TURNSTILE_SECRET … Turnstile を使う場合だけ（社長方針で当面使わない）
 *
 *   保存先が無いうちは、フォームは「受付を準備中」と正直に返す（黙って捨てない）。
 *
 * 🔴 onRequestPost だけを export する。onRequest を併記すると全メソッドを受けてしまい
 *    onRequestPost が呼ばれなくなる（Gin-DB で本番が405になった前例）。
 */

const MAX = { name: 100, email: 200, url: 500, message: 4000, kind: 40 };
const KINDS = ["掲載情報の誤り", "新規酒造所・銘柄の掲載依頼", "画像・引用について", "その他"];
...
async function ipHash(ip) {
  const buf = await crypto.subtle.digest("SHA-256", new TextEncoder().encode("awamoridb:" + ip));
  const a = new Uint8Array(buf);
  let s = "";
...
export async function onRequestPost({ request, env }) {
  ...
  // スパム対策5: 本文の判定。捨てずに「スパム判定」でシートへ回す
  ...
  if (env.CONTACT_WEBHOOK) {
    ...
    const payload = (env.CONTACT_WEBHOOK_FORMAT || "").toLowerCase() === "discord"
      ? { content: ["**泡盛DB お問い合わせ**", "```", ...lines, "```"].join("\n").slice(0, 1900) }
      : { site: "泡盛DB", kind, name, email, url, country, at, message, text: lines.join("\n") };
    try {
